...
class GameDataTableViewController: UITableViewController, DataReturn {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(UINib(nibName: "GameResultTableViewCell", bundle: nil), forCellReuseIdentifier: "GameResultCell")
    }
...
//            let next = storyboard.instantiateViewController(identifier: "Menu")
//            self.navigationController?.pushViewController(next, animated: true)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        default:
            let cell = tableView.dequeueReusableCell(withIdentifier: "GameDataCell", for: indexPath) as! GameResultTableViewCell
    ...
    @objc func moveToChangePlayerView(_ sender: UIButton) {
        ...
        let action3 = UIAlertAction(title: "プレイヤー交代", style: .default) { (_) in
            ...
            if let replaceVC = modalVC as? ReplacePlayerViewController {
                ...
                switch self.changePlayerCourtTag {
                case 11:
                    if let playingPlayerCourtFirst = self.courtFirstGamePlayers {
                        let waitingPlayer = self.gameDataModelBrain.getWaitingPlayers(playingPlayers: playingPlayerCourtFirst, playingPlayersInAnotherCourt: self.courtSecondGamePlayers, totalPlayers: self.players)

                        replaceVC.playingPlayer = playingPlayerCourtFirst
                        replaceVC.waitingPlayer = waitingPlayer
                    }

                case 22:
                    if let playingPlayerCourtSecond = self.courtSecondGamePlayers {
                        let waitingPlayer = self.gameDataModelBrain.getWaitingPlayers(playingPlayers: playingPlayerCourtSecond, playingPlayersInAnotherCourt: self.courtFirstGamePlayers, totalPlayers: self.players)

                        replaceVC.playingPlayer = playingPlayerCourtSecond
                        replaceVC.waitingPlayer = waitingPlayer
                    }
                default:
                    replaceVC.playingPlayer = []
                    replaceVC.waitingPlayer = []
                }
                self.present(replaceVC, animated: true)
            }
        }
            ...
            case 11:
                if let playersData = self.playerDataRecord.loadPlayers() {
                    ...
                    if self.gameDataFlag {
                        ...
                        let action1 = UIAlertAction(title: team1, style: .default) { (_) in
                        ...
                        let action2 = UIAlertAction(title: team2, style: .default) { (_) in
                        ...
                        let action3 = UIAlertAction(title: "引き分け", style: .default) { (_) in
