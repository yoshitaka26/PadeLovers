...
class MenuViewModel: BaseViewModel {
    ...
    let mainSettingButtonSelect = PublishSubject<Void>()
    let howToUseButtonSelect = PublishSubject<Void>()
    ...
    func mutate() {
        ...
        mainSettingButtonSelect.subscribe(onNext: { [weak self] _ in
            guard let self = self else { return }
            self.behaviorTransition.accept(.mainSetting)
        }).disposed(by: disposeBag)
        howToUseButtonSelect.subscribe(onNext: { [weak self] _ in
            guard let self = self else { return }
            self.behaviorTransition.accept(.howToUse)
        }).disposed(by: disposeBag)
    }
}
