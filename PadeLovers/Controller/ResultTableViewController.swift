...
class ResultTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ToDetailView" {
            let destinationVC = segue.destination as! ResultDetailTableViewController
            let index = tableView.indexPathForSelectedRow
            destinationVC.playerArrayNumber = index?.row
            switch index?.section {
            case 0:
                destinationVC.player = playingPlayers[index?.row ?? 0]
            case 1:
                destinationVC.player = restingPlayers[index?.row ?? 0]
            default:
                return
            }
        }
