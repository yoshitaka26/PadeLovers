...
struct CommonDataBrain {
    ...
    func savePlayers(group: TableType, players: [MasterPlayerDataModel]) {
    ...
    func loadPlayers(group: TableType) -> [MasterPlayerDataModel]? {
        var players = [MasterPlayerDataModel]()
        var filePath: URL?
        switch  group {
        case .group1:
            filePath = dataFilePathGroup1
        ...
        if let data = try? Data(contentsOf: path) {
            ...
            do {
                players = try decoder.decode([MasterPlayerDataModel].self, from: data)
            } catch {
